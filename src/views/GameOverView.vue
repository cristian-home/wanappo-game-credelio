...
const gameOverCardRef = ref<HTMLDivElement | null>()
...
const enterAnimation = () => {
  ...
  useMotion(gameOverCardRef.value, {
    initial: { opacity: 0, y: 100 },
...
const leaveAnimation = async () => {
  ...
  const cardMotion = useMotion(gameOverCardRef.value, {
    initial: { opacity: 1, y: 0 },
    leave: {
      opacity: 0,
      y: 100,
...
onBeforeRouteLeave(async (to, from, next) => {
  console.log('Leaving GameOverView, preparing animations...')
...
<template>
  ...
  >
    ...
    <div
      ...
      ref="gameOverCardRef"
    >
      <h1 class="text-yellow text-6xl font-bold">{{ t('game.gameOver') }}</h1>
      <p class="text-2xl text-white">{{ t('game.timeUp') }}</p>
